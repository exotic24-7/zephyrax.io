...
function spawnWave(waveNumber){
    ...
    for(let i=0;i<count;i++){
        ...
        if(cfg && cfg.mobs && cfg.mobs.length){
            ...
            let rarityIndex = Math.min(base + Math.floor(waveNumber/10), maxR);
            ...
            // choose a rarity index that slowly increases with wave
            const rarityIndex = Math.min(Math.floor(waveNumber/12), RARITY_NAMES.length-1);
            const rName = RARITY_NAMES[rarityIndex]; const mult = rarityMultiplier(rarityIndex);
...
function renderInventory(){
    ...
    items.forEach((it,idx)=>{
        ...
        d.innerHTML = `<div style=\"font-size:12px;text-align:center;padding-top:8px\">${label}</div><div style=\"position:absolute;right:4px;bottom:4px;font-weight:700;background:rgba(0,0,0,0.6);color:#fff;padding:2px 6px;border-radius:10px;\">${it.stack}x</div><div style=\"position:absolute;left:4px;top:4px;font-size:10px;padding:2px 4px;border-radius:4px;background:${rc};color:#000\">${rarity}</div>`;
        d.draggable = true;
    ...
    for(let si=0; si<10; si++){
        ...
        if(s){
            ...
            sd.textContent = def && (def.name||def.id) ? (def.name||def.id) : s.type;
            sd.title = (def && def.description) ? `${sd.textContent} - ${def.description} (${s.rarity||'Common'}) x${s.stack||1}` : ((s.rarity||'Common') + ' x' + (s.stack||1));
    ...
    if(main){
        ...
        for(let si=0; si<10; si++){
            ...
            if(s){
                const def = (window.PETAL_DEFS && (window.PETAL_DEFS[s.type] || window.PETAL_DEFS[(s.type||'').toLowerCase()])) || null;
                md.textContent = def && (def.name||def.id) ? (def.name||def.id) : s.type;
                md.title = (def && def.description) ? `${md.textContent} - ${def.description} (${s.rarity||'Common'}) x${s.stack||1}` : ((s.rarity||'Common') + ' x' + (s.stack||1));
    ...
    if(main){
        for(let i=0;i<10;i++){
            ...
            if(s && s.type && !s.empty){
                ...
                const name = document.createElement('div'); name.style.fontSize='12px'; name.textContent = label;
                el.appendChild(name);
